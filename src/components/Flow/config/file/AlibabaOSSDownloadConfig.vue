<template>
  <div class="alibaba-oss-download-config">
    <div class="config-content">
      <FlowConfigHero
        badge="文件"
        title="阿里云 OSS 下载"
        description="从阿里云 OSS 存储桶目录拉取文件到本地目录，并可配置下载后处理。"
        tone="amber"
      ...
      <template v-if="activeTab === 'basic'">
        <div class="form-item">
          ...
          <input v-model="formData.name" type="text" class="form-input" placeholder="阿里云OSS下载">
        </div>
...
<script>
import { aliyunOssDbTree, aliyunOssBucketList } from '@/api/collect/plugin/objectStorage'
...
export default {
  ...
  methods: {
    initDefaults() {
      if (!this.formData.name) this.$set(this.formData, 'name', '阿里云OSS下载')
    ...
    async loadDataSourceTree() {
      try {
        const res = await aliyunOssDbTree()
    ...
    normalizeDataSourceWithOptions() {
      ...
      this.dataSourceOptions = [
        { id: optId, name: `ID ${raw}（当前阿里云OSS数据源列表中未找到）` },
        ...this.dataSourceOptions
      ]
    ...
    async loadBucketOptions() {
      ...
      try {
        const res = await aliyunOssBucketList(dbId)
