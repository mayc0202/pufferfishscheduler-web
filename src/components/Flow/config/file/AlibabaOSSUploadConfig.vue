<template>
  <div class="alibaba-oss-upload-config">
    <div class="config-content">
      <FlowConfigHero
        badge="文件"
        title="阿里云 OSS 上传"
        description="将本地或流程产生的文件上传到阿里云 OSS 存储桶目录。"
        tone="amber"
      ...
      <template v-if="activeTab === 'basic'">
        <div class="form-item">
          ...
          <input v-model="formData.name" type="text" class="form-input" placeholder="阿里云OSS上传">
        </div>
...
export default {
  ...
  methods: {
    initDefaults() {
      if (!this.formData.name) this.$set(this.formData, 'name', '阿里云OSS上传')
