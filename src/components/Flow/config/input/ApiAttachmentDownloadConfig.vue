<template>
  <div class="api-attachment-download-config">
    <div class="config-content">
      <FlowConfigHero
        badge="集成"
        title="API 附件下载"
        description="调用接口下载附件到流程可用路径，便于后续文件类步骤处理。"
        tone="indigo"
      ...
      <template v-if="activeTab === 'basic'">
        <div class="form-item">
          ...
          <input v-model="formData.name" type="text" class="form-input" placeholder="API附件下载">
        </div>
        ...
        <div v-show="isPost" class="form-item">
      ...
      <template v-else>
        <div class="advanced-layout">
          ...
          <div class="section-header" @click="sectionOpen.page = !sectionOpen.page">
            <h4>分页请求<span class="help-icon" title="按后端约定拉取多页数据">?</span></h4>
            <div class="section-toggle">
              <i :class="sectionOpen.page ? 'el-icon-arrow-down' : 'el-icon-arrow-right'" />
            </div>
          </div>
          <div v-show="sectionOpen.page" class="section-content">
            <div class="form-item checkbox-item">
              <el-checkbox v-model="formData.usePage">启用分页</el-checkbox>
            </div>
            <div v-if="formData.usePage" class="form-item">
              <label class="form-label">分页条件：</label>
              <textarea
                v-model="formData.pageCondition"
                class="form-textarea"
                rows="3"
                placeholder="分页参数（如 JSON，以后端为准）"
              />
            </div>
            <div v-if="formData.usePage" class="form-item">
    ...
    <el-dialog
      ...
      append-to-body
      :close-on-click-modal="false"
      custom-class="api-input-preview-dialog"
    >
...
export default {
  name: 'ApiAttachmentDownloadConfig',
  ...
  methods: {
    ...
    initDefaults() {
      if (!this.formData.name) this.$set(this.formData, 'name', 'API附件下载')
    ...
    buildStepData() {
      ...
      const useRaw = !!this.formData.useRaw
      const useXForm = useRaw ? false : this.formData.useXForm !== false

      const headerList = (this.formData.headerList || []).map(normalizeHeaderRow).filter(r => r.name || r.value || r.description)
      const xFormList = useXForm
        ? (this.formData.xFormList || []).map(normalizeParamRow).filter(r => r.key || r.value || r.description)
        : []
      ...
      return {
        ...
        requestMethod: this.formData.requestMethod != null ? String(this.formData.requestMethod) : '',
        requestType:
          this.isPost && this.formData.requestType != null ? String(this.formData.requestType) : '',
        ...
        pageCondition:
          this.formData.usePage && this.formData.pageCondition != null
            ...
            : '',
        startPageNo: this.formData.usePage
          ? Math.max(1, Math.trunc(Number(this.formData.startPageNo)) || 1)
          : 1
      }
    },
    validateForSubmit() {
      ...
      if (String(d.requestMethod).toUpperCase() === 'POST' && !d.requestType) {
        this.$message.warning('POST 请求请选择请求类型')
        return false
      }
      if (!d.responseStatus) {
...
<style scoped>
.api-attachment-download-config {
  width: 100%;
}
...
.api-attachment-download-config ::v-deep .api-param-table--fill.el-table {
  width: 100% !important;
}

.api-attachment-download-config ::v-deep .api-param-table--fill .el-table__header-wrapper table,
.api-attachment-download-config ::v-deep .api-param-table--fill .el-table__body-wrapper table {
  width: 100% !important;
  table-layout: fixed;
}

.api-attachment-download-config ::v-deep .api-param-table .cell {
  text-align: center;
}

.api-attachment-download-config ::v-deep .api-param-table .el-table__empty-text {
  display: block;
  width: 100%;
  text-align: center;
}

.api-attachment-download-config ::v-deep .api-param-table .el-input__inner {
  text-align: center;
}
