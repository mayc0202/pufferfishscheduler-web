<template>
  <div class="redis-output-config">
    <FlowConfigHero
      badge="Redis"
      title="Redis 输出"
      description="将流中的值写入 Redis 指定 Key；连接信息来自数据源，与后端 RedisOutput 步骤字段一致。"
      tone="coral"
    ...
    <div class="config-body">
      ...
      <template v-if="activeTab === 'basic'">
        <div class="form-item">
          ...
          <input v-model="formData.name" type="text" class="form-input" placeholder="Redis输出">
        </div>
        ...
        <div class="form-item">
          ...
          <el-select
            ...
            popper-class="redis-output-db-popper"
        ...
        <div class="form-item">
          <label class="form-label">数据库 / DB 名称</label>
          <el-input
            v-model="formData.dbName"
            class="redis-el-input"
            clearable
            placeholder="对应后端 dbName，如逻辑库索引 0；留空则按插件默认"
          >
...
export default {
  ...
  methods: {
    initDefaults() {
      if (!this.formData.name) this.$set(this.formData, 'name', 'Redis输出')
      ...
      if (this.formData.dbName === undefined) this.$set(this.formData, 'dbName', '')
      if (this.formData.key === undefined) this.$set(this.formData, 'key', '')
      if (this.formData.value === undefined) this.$set(this.formData, 'value', '')
    ...
    buildPayloadForSave() {
      ...
      raw.dbName = raw.dbName != null ? String(raw.dbName).trim() : ''
      raw.key = raw.key != null ? String(raw.key).trim() : ''
      raw.value = raw.value != null ? String(raw.value) : ''
      raw.copiesCache =
    ...
    handleSubmit() {
      ...
      if (!String(this.formData.value || '').trim()) {
        this.$message.warning('请填写写入 Redis 的 Value')
        return
      }
...
.redis-el-textarea ::v-deep .el-textarea__inner {
  border-radius: 4px;
  font-size: 14px;
  border-color: #dcdfe6;
}

.redis-el-textarea ::v-deep .el-textarea__inner:focus {
  border-color: var(--flow-primary);
}
