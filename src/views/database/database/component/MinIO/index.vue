  ...
  <el-container>
    ...
    <el-main class="scrollable-main">
      ...
      >
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="S3 API 地址" prop="endpoint" class="db-form-item">
              <el-input
                v-model="databaseInfo.endpoint"
                size="mini"
                placeholder="例如 http://127.0.0.1:9000 或 https://minio.example.com"
                clearable
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="Region" prop="region" class="db-form-item">
              <el-input
                v-model="databaseInfo.region"
                size="mini"
                placeholder="默认 us-east-1（与 MinIO 常见配置一致）"
                clearable
              />
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="Access Key" prop="accessKeyId" class="db-form-item">
              <el-input
          ...
          <el-col :span="12">
            <el-form-item label="Secret Key" prop="accessKeySecret" class="db-form-item">
              <el-input
        ...
        <el-row :gutter="16">
          <el-col :span="12">
            <el-form-item label="默认存储桶（可选）" class="db-form-item">
              <el-input
                v-model="databaseInfo.defaultBucket"
                size="mini"
                placeholder="可选，不填则连接时不校验具体桶"
                clearable
              />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="连接选项" class="db-form-item minio-options">
              <div class="option-row">
                <span class="option-label">Path-Style</span>
                <el-switch v-model="databaseInfo.pathStyleAccess" active-text="开启" inactive-text="关闭" />
              </div>
              <div class="option-hint">MinIO 自建集群通常需开启 Path-Style</div>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="16">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        endpoint: [{ required: true, trigger: 'blur', message: '请输入 S3 API 地址!' }],
        accessKeyId: [{ required: true, trigger: 'blur', message: '请输入 Access Key!' }],
        accessKeySecret: [{ required: true, trigger: 'blur', message: '请输入 Secret Key!' }],
  ...
  methods: {
    ...
    async buildPayload() {
      ...
      database.dbPort = ''
      database.dbName = database.defaultBucket || ''
      database.dbSchema = ''
      database.properties = {
        region: database.region || 'us-east-1',
        pathStyleAccess: database.pathStyleAccess !== false,
    ...
    loadDetail(dbId) {
      detailDb(dbId).then((res) => {
        ...
        this.databaseInfo = {
          ...
          defaultBucket: detail.dbName || '',
          pathStyleAccess: properties.pathStyleAccess !== false,
