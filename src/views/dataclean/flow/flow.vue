        ...
        <el-aside width="260px" class="page-aside">
          <div class="group">
            <div class="wrap">
              ...
              <el-button type="primary" class="queryAll" size="mini" @click="queryGroupList()">查看全部数据</el-button>
            ...
            <div class="list">
              <div class="search">
                <el-row class="search-row search-row-uniform" :gutter="12">
                  <el-col class="search-col search-col-equal" :span="8">
                    <div class="col">
                  ...
                  <el-col class="search-col search-col-equal search-col-btns" :span="16">
                    <div class="col search-btns">
                      <el-button type="primary" icon="el-icon-search" size="small" @click="queryFlowList()">查询</el-button>
                      <el-button type="primary" size="small" @click="openFlowDialog()">新增清洗流程</el-button>
                    </div>
                  </el-col>
                </el-row>
              </div>
...
// 新增：导入流程相关API
import { list, addFlow, updateFlow, deleteFlow as deleteFlowApi, copyFlow as copyFlowApi } from '@/api/collect/trans/transFlow'
...
export default {
  ...
  methods: {
    ...
    async copyFlow(row) {
      try {
        const res = await copyFlowApi(row.id)
        if (res.code === '000000') {
          this.$message.success('流程复制成功！')
          // 刷新流程列表
          this.queryFlowList()
        } else {
          this.$message.error(res.message || '流程复制失败')
        }
      } catch (error) {
        console.error('复制流程失败:', error)
        this.$message.error('流程复制失败')
      }
    },
